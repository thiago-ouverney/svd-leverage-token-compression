...
Materializamos o operador de poda inspirado no SVD-Prune em um pipeline reprodutível sobre reviews IMDb. Comparamos operadores (\texttt{svd}, \texttt{svd\_energia} e baselines) sob o mesmo par $(\varepsilon, \rho)$, reportando acurácia downstream, compressão $C$ e, para métodos SVD, energia espectral $E_k^{\mathrm{energia}}$ e fidelidade de reconstrução $R_k$.
...
No pipeline (Seção~\ref{sec:pipeline}), treino e teste passam pelo \textbf{mesmo} operador por documento; o rótulo de sentimento só entra \textbf{depois} da poda, no classificador ridge --- a SVD permanece não supervisionada. Neste relatório não avaliamos variantes corpus-wide.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{(A) Métricas algébricas}: compressão $C$; para SVD, $E_k^{\mathrm{energia}}$ e $R_k$ sobre $F_k$.
...
\begin{itemize}
    \item \textbf{full}: mantém todos os tokens (referência sem poda).
...
Treino e teste passam pelo \textbf{mesmo operador} e $\rho$ antes da agregação e da classificação; \texttt{full} serve de referência sem poda. Comparamos métodos à mesma $\rho$: menor queda de acurácia em relação a \texttt{full} indica menor perda no proxy supervisionado (Seção~\ref{sec:intro}).
...
Reportamos os dois proxies da Seção~\ref{sec:intro}: \textbf{(A)} algébrico ($C$, $E_k^{\mathrm{energia}}$, $R_k$) e \textbf{(B)} supervisionado (acurácia downstream).
...
\begin{description}
    ...
    \item[Fidelidade de reconstrução]
    \begin{equation*}
        R_k = 1 - \frac{\|F - F_k\|_F}{\|F\|_F}.
    \end{equation*}
    Mede o erro relativo de $F_k = U_k \Sigma_k V_k^\top$ em norma de Frobenius. Reportada para métodos SVD; depende de $F_k$, não da seleção $S$; baselines registram $\mathrm{NaN}$.
\end{description}
...
\begin{table}[ht]
    ...
    \begin{tabular}{ll}
        ...
        $R_k$ & fidelidade de reconstrução ($F_k$ vs.\ $F$) \\
        \hline
    \end{tabular}
\end{table}
